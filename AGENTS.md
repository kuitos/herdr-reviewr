...
- `src/walk.rs` — the `All files` listing outside a git repo: an `ignore` walk with `ls-files`'s shape (`.gitignore` honored, ignored dirs as placeholders), capped at `LISTING_CAP`. `world::build` picks it when `RepoKind` is `Plain`; a plain directory has no changeset, and the git-only surfaces answer `not a git repository`.
